...
connection, the Mosh client is able to predict the application's
behavior in response to user keystrokes and (if confident in its
predictions) can speculatively display the result---generally an
echoed keystroke or cursor motion---before it has been confirmed by
the server. In practice, in a trace-based evaluation using 40 hours of
real-world usage, Mosh was able to show immediately the results of
...
independent users may contend for the same limited network resource. Remy
